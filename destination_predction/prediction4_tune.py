...
# Generate all possible n-nucleotides (e.g., tetranucleotides) from 'ACGT'
...
def count_nucleotides(genome_sequence, n):
    ...
    Parameters:
        ...
        n: Length of n-nucleotide (e.g., 4 for tetranucleotides).
    Returns:
...
def prepare_data(cols_file):
    ...
    """
    rows_file = 'chr126_4_dra_both.csv'  # File containing row data
...
def train_and_predict(input_file, output_directory):
    ...
    # Load training data
    train_data = pd.read_csv("data_with_labels.csv", delimiter=",", skiprows=0, usecols=range(0, 257))
    X_train = train_data.iloc[:, :-1]  # Features
    y_train = train_data.iloc[:, -1]   # Labels

    # Train the Random Forest model
    model = RandomForestClassifier(n_estimators=200, min_samples_leaf=1, min_samples_split=5, random_state=42)
    model.fit(X_train, y_train)

    # Load test data
    test_data = pd.read_csv(input_file, delimiter=",", skiprows=0, usecols=range(0, 256))
...
if __name__ == "__main__":
    ...
    # Step 1: Count n-nucleotides (e.g., tetranucleotides)
    n = 4  # Length of the nucleotide sequences
    all_nucleotides = generate_all_nucleotides(n)  # Generate all possible tetranucleotides
    nucleotide_counts = process_fna_file(input_fasta_file, n)  # Count tetranucleotides in the input file

    # Save the n-nucleotide counts to a CSV file
    output_file = os.path.join(output_directory, 'tetranucleotide_counts.csv')
    ...
    transposed_output_file = os.path.join(output_directory
