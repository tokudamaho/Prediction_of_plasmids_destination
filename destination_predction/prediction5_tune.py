...
# Generate all possible pentanucleotides from 'ACGT'
def generate_all_pentanucleotides():
    """Generate all possible pentanucleotides (5-mers) from 'ACGT'."""
    return [''.join(p) for p in itertools.product('ACGT', repeat=5)]
...
# Count occurrences of pentanucleotides in a genome sequence (including reverse complement)
def count_pentanucleotides(genome_sequence):
    """
    Count pentanucleotide frequencies in the given sequence and its reverse complement.

    Parameters:
        genome_sequence: DNA sequence as a string.
    Returns:
        Dictionary with pentanucleotide relative frequencies.
    """
    pentanucleotide_counts = {}
    total_pentanucleotides = 0
    sequences = [genome_sequence, reverse_complement(genome_sequence)]  # Include both strands
    for seq in sequences:
        for i in range(len(seq) - 4):  # Slide over the sequence to extract pentanucleotides
            pentanucleotide = seq[i:i+5]
            if set(pentanucleotide) <= set("ACGT"):  # Only count valid ACGT pentanucleotides
                total_pentanucleotides += 1
                pentanucleotide_counts[pentanucleotide] = pentanucleotide_counts.get(pentanucleotide, 0) + 1
    # Normalize counts to relative frequencies
    for pentanucleotide in pentanucleotide_counts:
        pentanucleotide_counts[pentanucleotide] /= total_pentanucleotides
    return pentanucleotide_counts

# Process a .fna file to count pentanucleotides
def process_fna_file(file_path):
    """
    Extract DNA sequence from a .fna file and count pentanucleotides.

    Parameters:
        file_path: Path to the input .fna file.
    Returns:
        Dictionary with pentanucleotide frequencies.
    """
    try:
        with open(file_path, 'r') as file:
            ...
            return count_pentanucleotides(genome_sequence)
...
def prepare_data(cols_file):
    ...
    """
    rows_file = 'data/chr127_5_dra_both.csv'  # File containing row data
    ...
    temp_output_file = os.path.join(output_directory, 'temp_testdata5.csv')
...
def train_and_predict(input_file, output_directory):
    ...
    # Load training data
    train_data = pd.read_csv("data/data_with_labels52.csv", delimiter=",", usecols=range(0, 1025))
    X_train = train_data.iloc[:, :-1]  # Features
    y_train = train_data.iloc[:, -1]   # Labels

    # Train the Random Forest model
    model = RandomForestClassifier(max_depth=20, n_estimators=100, min_samples_leaf=1, min_samples_split=2, random_state=42)
    model.fit(X_train, y_train)

    # Load test data
    test_data = pd.read_csv(input_file, delimiter=",", usecols=range(0, 1024))
    predictions = model.predict(test_data)

    # Save predictions to a CSV file
    predictions_file = os.path.join(output_directory, "Predictions5.csv")
    ...
    importance_file = os.path.join(output_directory, "Feature_Importances5.csv")
...
if __name__ == "__main__":
    ...
    transposed_output_file = "transposed5.csv"  # Fixed file name for transposed output

    # Ensure the output directory exists
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)

    # Step 1: Count pentanucleotides
    pentanucleotide_counts = process_fna_file(input_fasta_file)

    # Save pentanucleotide counts to a CSV file
    counts_file = os.path.join(output_directory, 'pentanucleotide_counts.csv')
    pd.DataFrame.from_dict(pentanucleotide_counts, orient='index', columns=['Frequency']).reset_index() \
        .rename(columns={'index': 'Pentanucleotide'}).to_csv(counts_file, index=False)
    print(f"Pentanucleotide counts written to {counts_file}")
